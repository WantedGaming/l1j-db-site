<?php
/**
 * Admin Accessories List for L1J Database Website
 */
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    $accessoryId = intval($_GET['id']);
    
    // Add confirmation check with POST for security
    if (isset($_POST['confirm_delete']) && $_POST['confirm_delete'] == 'yes') {
        // Delete the accessory
        $result = $db->query("DELETE FROM accessories WHERE item_id = ?", [$accessoryId]);
        
        if ($result) {
            // Set success message and redirect
            $_SESSION['admin_message'] = ['type' => 'success', 'message' => 'Accessory deleted successfully.'];
        } else {
            // Set error message
            $_SESSION['admin_message'] = ['type' => 'error', 'message' => 'Failed to delete accessory.'];
        }
...
// Build query for accessories list
...
// Get total count
$totalAccessories = $db->getColumn("SELECT COUNT(*) FROM accessories");
$totalPages = ceil($totalAccessories / $itemsPerPage);
...
// Execute query
$accessories = $db->getRows($query, $params);
?>
...
<div class="admin-container">
    ...
    <div class="admin-hero-container">
        <div class="admin-hero-content">
            <h1 class="admin-hero-title">Manage Accessories</h1>
            <p class="admin-hero-subtitle">Total Accessories: <?= $totalAccessories ?></p>
    ...
    <!-- Accessories List Table - Wrapped in constrained width -->
                    ...
                    <?php else: ?>
                        <?php foreach ($accessories as $accessory): ?>
                            <tr>
                                ...
                                <td>
                                    <img src="<?= SITE_URL ?>/assets/img/items/<?= $accessory['icon_id'] ?>.png" 
                                ...
                                <td><?= htmlspecialchars($accessory['type'] ?? 'Standard') ?></td>
                                <td><?= htmlspecialchars($accessory['grade'] ?? 'Normal') ?></td>
                                <td><?= $accessory['weight'] ?></td>
                                <td class="actions">
                                    <a href="edit.php?id=<?= $accessory['item_id'] ?>" class="btn btn-sm btn-edit" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="#" class="btn btn-sm btn-delete" title="Delete" 
                                       onclick="confirmDelete(<?= $accessory['item_id'] ?>, '<?= addslashes($accessory['name']) ?>')">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                    <a href="<?= SITE_URL ?>/pages/accessories/accessory-detail.php?id=<?= $accessory['item_id'] ?>" class="btn btn-sm btn-view" title="View" target="_blank">
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination">
            <div class="pagination-info">
                Showing <?= ($offset + 1) ?>-<?= min($offset + $itemsPerPage, $totalAccessories) ?> of <?= $totalAccessories ?> accessories
            </div>
...
<div id="deleteModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p>Are you sure you want to delete the accessory: <span id="deleteItemName"></span>?</p>
