...
// Load doll data
$doll = $db->getRow("SELECT * FROM dolls WHERE id = ?", [$dollId]);

// Redirect if doll not found
if (!$doll) {
    $_SESSION['admin_message'] = ['type' => 'error', 'message' => 'Doll not found.'];
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Required fields
    if (empty($updatedDoll['name'])) {
        $errors[] = "Doll name is required";
    }
    
    if (empty($updatedDoll['item_id'])) {
    ...
    if (empty($errors)) {
        ...
        foreach ($updatedDoll as $field => $value) {
            $setParts[] = "$field = ?";
            $params[] = $value;
        }
        
        // Add the WHERE parameter (doll ID)
        $params[] = $dollId;
        
        $query = "UPDATE dolls SET " . implode(', ', $setParts) . " WHERE id = ?";
        ...
        if ($result) {
            ...
            $_SESSION['admin_message'] = [
                'type' => 'success',
                'message' => "Doll '{$updatedDoll['name']}' updated successfully."
            ];
            ...
            $errors[] = "Failed to update doll. Database error.";
        }
    }
    
    // Update the doll variable with form data if there were errors
    if (!empty($errors)) {
        $doll = array_merge($doll, $updatedDoll);
    }
}
...
<div class="admin-container">
    ...
    <div class="admin-content-card">
        <div class="admin-content-header">
            <h2>Edit Doll: <?= htmlspecialchars($doll['name']) ?></h2>
        </div>
        ...
        <form method="POST" action="edit.php?id=<?= $dollId ?>" class="admin-form">
            <div class="form-grid">
                <!-- Basic Information -->
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="id" value="<?= $doll['id'] ?>" class="form-control" disabled>
                        <span class="form-text">Unique identifier for this doll (cannot be changed)</span>
                    </div>
